...
def TransportBit(app: Application, pool: Any, id: int, config: str) -> Tuple[Any, Any]:
    """
    Create a transport bit for receiving network data.
    ...
    max_packet_size = TRANSPORT_HEADER_SIZE + VOLTAGE_HEADER_SIZE + VOLTAGE_DATA_SIZE
...
def SourceBit(app: Application, pool: Any, id: int, config: str) -> Tuple[Any, Any]:
    """
    Create a source bit for generating test data.
    ...
    source_name = f"transport-source-{id}"
    source_op = SourceOp(
        fragment=app,
        name=source_name,
        total_block=total_block_shape,
